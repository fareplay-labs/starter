...
export function useBackendService() {
  ...
  const loadUserCasino = useCallback(
    async (userId: string, force = false): Promise<CasinoEntity | null> => {
      ...
      try {
        const casino = await BackendService.loadUserCasino(userId, force)
        setIsLoading(false)
        return casino
      } catch (err) {
  ...
  const saveUserCasino = useCallback(
    async (userId: string, casino: CasinoEntity): Promise<boolean> => {
      ...
      try {
        await BackendService.saveUserCasino(userId, casino)
        setIsLoading(false)
        return true
      } catch (err) {
  ...
  const loadGameConfig = async (
    ...
    try {
      const config = await BackendService.getGameConfig(username, instanceId, gameType)
      setIsLoading(false)
      return config
    } catch (err) {
  ...
  const saveGameConfig = async (
    ...
    try {
      await BackendService.saveGameConfig(username, instanceId, config, gameType)
      setIsLoading(false)
      return true
    } catch (err) {
  ...
  const getCasinoPreviews = useCallback(async (): Promise<CasinoPreview[]> => {
    ...
    try {
      const previews = await BackendService.getCasinoPreviews()
      setIsLoading(false)
      return previews
    } catch (err) {
  ...
  const getFeaturedCasinoPreviews = useCallback(
    async (limit = 3): Promise<FeaturedCasinoPreview[]> => {
      ...
      try {
        const featured = await BackendService.getFeaturedCasinoPreviews(limit)
        setIsLoading(false)
        return featured
      } catch (err) {
  ...
  const clearAllUserData = useCallback(async (userId: string): Promise<void> => {
    ...
    try {
      await BackendService.clearAllUserData(userId)
      setIsLoading(false)
    } catch (err) {
  ...
  const createGame = useCallback(
    async (userId: string, type: string, name: string): Promise<{ id: string; type: string; name: string } | null> => {
      ...
      try {
        const game = await BackendService.createGame(userId, type, name)
        setIsLoading(false)
        return game
      } catch (err) {
  ...
  const deleteGameFromCasino = useCallback(async (userId: string, gameId: string): Promise<boolean> => {
    ...
    try {
      await BackendService.deleteGameFromCasino(userId, gameId)
      setIsLoading(false)
      return true
    } catch (err) {
