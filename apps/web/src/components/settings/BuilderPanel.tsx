    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Generate a theme and assets for your casino with AI.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Describe the visual style, colors, and tone.
            </p>
          </div>
        ...
        <CardContent className="space-y-2">
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {jobs.map((j) => (
              <div
                key={j.id}
                className={`border rounded p-3 ${selectedJob?.id === j.id ? 'border-primary' : 'border-border'}`}
              >
                  ...
                  <div className="mt-2 text-xs">
                    <div>
                      ...
                      <span className="font-mono">
                        {j.result.theme.primaryColor}
                      </span>
                    </div>
                    {j.result.assets?.logoUrl && (
                      <div>
                        Logo:{' '}
                        <a
                          href={j.result.assets.logoUrl}
                          target="_blank"
                          rel="noreferrer"
                          className="underline"
                        >
                    ...
                    {j.result.assets?.bannerUrl && (
                      <div>
                        Banner:{' '}
                        <a
                          href={j.result.assets.bannerUrl}
                          target="_blank"
                          rel="noreferrer"
                          className="underline"
                        >
      ...
      <BuilderModal
        ...
        onApplyTheme={
          selectedJob?.status === 'COMPLETE' && onApplyTheme && selectedJob?.id
            ? async () => {
                await onApplyTheme(selectedJob.id);
              }
            : undefined
        }
