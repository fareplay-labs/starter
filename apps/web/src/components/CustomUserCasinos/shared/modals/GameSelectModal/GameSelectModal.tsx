...
import { DEFAULT_GAMES } from '@/components/CustomUserCasinos/UserPage/GameSections/defaultGames'
import { useSectionManagement } from '@/components/CustomUserCasinos/backend/hooks/useSectionManagement'
import { useBackendService } from '@/components/CustomUserCasinos/backend/hooks'
import useNotiStore from '@/store/useNotiStore'
import {
  SmallTile,
  GameIcon,
} from '@/components/CustomUserCasinos/UserPage/GameSections/components'
    ...
    try {
      ...
      if (userId) {
        try {
          ...
          await RealBackendService.deleteGameFromCasino(userId, confirmDelete.gameId)
    ...
    if (casinoConfig?.sections) {
      ...
      if (sectionIndex !== -1) {
        ...
        if (userId && oldLayout !== selectedLayout) {
          try {
            const RealBackendService = (await import('@/components/CustomUserCasinos/backend/core/RealBackendService')).RealBackendService
            await RealBackendService.updateSection(userId, sectionId, { layout: selectedLayout })
          } catch (error) {
